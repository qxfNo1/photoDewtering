...
import networks
photoDewatering = Blueprint('photoDewatering',__name__)
...
@photoDewatering.route('/photoDewatering', methods=['GET', 'POST'])
def dewatering(name=None):
    ...
    if request.method == 'POST':
        if 'file' in request.files:
            ...
            if file and allowed_file(file.filename):
                ...
                return render_template('photoDewatering.html', name=name, image_name=filename, image_width=W, image_height=H,comment_list=comment_list,total=total,method=method)
            else:
                filename='example.jpg'
                image = Image.open(os.path.join(imgPath, filename))
                W, H = image.size
                return render_template('photoDewatering.html', name=name, image_name=filename, image_width=W, image_height=H, is_alert=True,comment_list=comment_list,total=total,method=method)
        if 'mask' in request.form:
            ...
            return render_template('photoDewatering.html', name=name, image_name0=filename, image_name="result_" + maskname, mask_name=maskname, image_width=W, image_height=H,comment_list=comment_list,total=total,method=method)
    else:
        filename='example.jpg'
