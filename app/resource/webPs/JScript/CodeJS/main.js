...
//打开文件
function fileOpen() {
    showMyWindow('icon-open', '打开', '/webPs/Code/SelectImg.html', 800, 400, true, false, false, true);
}

//获取
function imgFromCamera() {
    showMyWindow('icon-fromCam', '获取', '/webPs/Code/GetCamareImg.aspx', 800, 600, true, false, false, true);
}
    ...
    if (this.id != undefined) {
        ...
        switch (oprationid) {
            case "mnHelphelp": //WebPhotoshop帮助
                window.open("/webPs/Code/Help.htm");
                break;
            case "mnHelphelpsite": //WebPhotoshop网站
                window.open("https://www.shu.edu.cn/info/1296/51098.htm");
                break;
            case "mnHelpabout": //关于WebPhotoshop
                showMyWindow('icon-about', '关于WebPhotoshop', '/webPs/Code/About.htm', 600, 400, true, false, false, true);
                break;
        }
    }
});
...
function ToolOpration(oprationid, oprationtitle) {
    ...
    $("#objectimg").attr("src", "/webPs/Images/tools/" + oprationid.substring(3, oprationid.length) + ".png");
    switch (oprationid) {
        case "btnSelect": //选择工具
